...
#   - 祖先 SHA が失効しても（ベース側の force-push / filter-branch）、マーカーの applied_at から
#     近似の祖先を解決してマージを続ける。それも取れない場合は上書きに落とさず下流を温存する。
#     初回適用・ベース切替・--no-merge で上書きしたファイルは「何を失ったか」を必ず一覧報告する
#     （黙って消えるのを構造的に無くす・Issue #233）。
set -euo pipefail
