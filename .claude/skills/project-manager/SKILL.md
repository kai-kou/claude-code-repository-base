...
gh project field-list PROJECT_NUMBER --owner __OWNER__ --format json
# 2. アイテムID を取得
gh project item-list PROJECT_NUMBER --owner __OWNER__ --format json
# 3. ステータスを更新
...
```bash
# マイルストーン別進捗
gh api repos/__OWNER__/__REPO__/milestones --jq '.[] | "\(.title): \(.closed_issues)/\(.open_issues + .closed_issues) done"'

# ステータス別の Issue 一覧
gh issue list --label "status:in-progress" --state open --repo __OWNER__/__REPO__
gh issue list --state all --limit 100 --repo __OWNER__/__REPO__
...
```bash
gh api repos/__OWNER__/__REPO__/milestones --jq '.[] | "\(.number): \(.title)"'
gh api --method PATCH repos/__OWNER__/__REPO__/milestones/MILESTONE_NUMBER -f state="closed"
```
