...
```bash
SKIP=$(gh issue view {ISSUE_NUM} -R __OWNER__/__REPO__ --json labels \
...
if [ "$NEXT" -lt 3 ]; then
  ...
  gh label create "research-skip:${NEXT}" -R __OWNER__/__REPO__ --color FBCA04 2>/dev/null || true
  [ "$SKIP" -gt 0 ] && gh issue edit {ISSUE_NUM} -R __OWNER__/__REPO__ --remove-label "research-skip:${SKIP}"
  gh issue edit {ISSUE_NUM} -R __OWNER__/__REPO__ \
