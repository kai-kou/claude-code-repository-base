...
**変更内容**:
- `tools/jev_client.py`（標準ライブラリのみ・opt-in・fail-soft・モデル ID 固定）を新設。`tools/triage_notification.py` は正規表現で非 A と判定した通知だけを Jev に問い合わせ、A-1〜A-6 の言い換え（キーワード非一致）を A として拾う 2 段構成になった（A → 非 A の取り消しはしない非対称設計。`--self-test` は従来どおり決定論、`--self-test-jev` が実 API 検証）
- **Jev が使えないとき（キー未設定・不通・429/529・タイムアウト・不正応答）は従来の正規表現判定にそのまま倒れる**。連続失敗時はプロセス内サーキットブレーカーが以降の呼び出しを止め、バッチ処理がタイムアウト待ちで遅くならない
- 活用ガイド `docs/jev-integration.md`（向き不向き・有効化手順・フォールバック設計・下流プロジェクト種別ごとの活用パターン・ガードレール G-1〜G-8）を配布物に追加
- TypeSafe 公式エージェントスキル `.claude/skills/typesafe-ai/`（MIT・そのまま同梱）を追加。Jev 統合を実装するセッションが公式ライブドキュメントへ誘導される
- `docs/rules/env-vars.md` に `JEV_KEY`（任意）を追記

**下流で必要な手動手順**:
1. Jev を使う場合のみ: 環境変数 `JEV_KEY` を Claude.ai 環境変数（または GitHub Variables）に設定し、`python3 tools/jev_client.py --ping` で疎通を確認する。**未設定なら何も変わらない**（全処理が従来どおり Jev なしで動く）
2. 自プロジェクトでの活用箇所は `docs/jev-integration.md` §4 を自分のドメインに読み替えて決め、§4.5 の順序（シャドー運用 → 方向と閾値の決定 → 非対称に採用）で入れる。フック（同期実行）への組み込みは運用実績が出るまで行わない（§5 G-8）
3. 送る内容が第三者（米国ホスト）に渡るため、データ方針上 ZDR が必要なプロジェクトは採用しない（§5 G-7）
...
## 2026-09-05（Issue #578）作業領域外への Bash 書き込みを機械ブロックするガードを追加

- **変更内容**: `.claude/hooks/lib/workspace_write_guard.py` を新設し `pre-tool-use-router.sh` から配線した。
  作業ディレクトリ・セッション一時領域の外への書き込み / 削除と、ホーム配下の `.claude` 領域への Bash アクセスを
  **承認プロンプトになる前に exit 2 で差し戻す**。クラウド実行環境ではサンドボックスが起動できず（実測）、
  作業ツリー外を触る Bash は auto モードの classifier が自動承認しないため、無人ルーティンが承認待ちのまま
  無限停止していた（根本原因と不採用案は `docs/rules/lessons/permissions.md` L-129）。
- **下流で必要な手動手順**: ① `CLAUDE.md`「やってはいけないこと」に 1 行追加している（一時作業ディレクトリを
  作業ツリー外に作らない・persisted output はネイティブ Read / Grep で読む）。**下流の `CLAUDE.md` は
  プロジェクト固有ファイルとして保護されるため自動反映されない**。同じ規範を自リポジトリの `CLAUDE.md` へ
  転記すること（フックの機械ブロックだけでも停止は防げるが、着手前の規範があると 1 往復のロスを避けられる）。
  ② 作業ツリー外の固定パス（`/tmp/{固定名}` 等）を前提にしたスキル・スクリプトを持つ下流は、
