...
>   --pr-url "https://github.com/__OWNER__/__REPO__/pull/{N}" \
...
2. 【必須・L-050】PR作成直後の存在確認（L-050 対策）
   ...
   ローカル: gh pr list --head {現在のブランチ名} -R __OWNER__/__REPO__ \
...
```
# ① PRイベント購読を開始
mcp__github__subscribe_pr_activity(owner="__OWNER__", repo="__REPO__", pull_number={pr_number})
...
```bash
# GitHub MCP ツールでマージ
mcp__github__merge_pull_request(owner="__OWNER__", repo="__REPO__", pull_number=N, merge_method="squash")
```
...
4. 【必須・L-050】PR作成直後に存在確認: クラウドは `mcp__github__list_pull_requests(head="{owner}:{ブランチ}", state="open")` / ローカルは `gh pr list --head {ブランチ名} -R __OWNER__/__REPO__ --limit 1 --json number,url,state` → URL を確認してから報告する
