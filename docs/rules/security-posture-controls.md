...
>
> **既知の未対応**（#395）: 候補トークンは各コマンドの **第1非フラグ引数** とリダイレクト先に限られるため、
> `tar czf out.tgz ~/.ssh`（第2引数以降）や `install -m 600 ~/.ssh/id_rsa /tmp/x`（値を取るフラグでずれる）は
> 素通りする。読み取り元と書き込み先を区別できないまま全引数を候補にすると誤検知が増えるため、設計を分けて検討中。
