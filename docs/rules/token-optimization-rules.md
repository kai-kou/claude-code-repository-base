...
| 日付 | 実測 | 差分 | 追加の正当化 / 相殺 |
|---|---:|---:|---|
| 2026-09-13 | 86,683 B | 基準 | **#648 の再棚卸し後の到達値**（増減ログ 4 行到達の合図 ① を受けての再点検。前回基準（2026-09-05・85,521B）からの増分 +1,162B・+1.4% は超過閾値（+10%）に遠い）。前回棚卸し以降に Hot 実体が変わったのは 4 本のみで（`lessons-core.md`（#618 の L-130 索引・#648 の L-132 索引）/ `pr-review-flow-summary.md`（#627 のレビュー方針言い換え）/ `completion-report-rules.md`（#633 の再掲抑止 1 行）/ `agent-team-summary.md`（#648 のポーリング禁止の手段非依存化））、残る 9 本は #580 時点から無変更のため当時の点検結果（削減対象 0 件）がそのまま有効。変わった 4 本の増分はいずれも削減対象外②（実観測ベースの行動規範）で、機械強制レイヤが代替しない（#633 はマーカーが実質機能していなかった実績、#648 は `sleep` 限定の禁止表現が `ReadNotifications` 連打を素通りさせた実観測）。削減対象は 0 件のため、増減ログをここで打ち切り本行を新基準とする（`tools/check_hot_budget.py` で機械検証） |
| 2026-09-16 | 87,871 B | +1,188 B | #678（秘密検知ゲート）: `lessons-core.md` の Warm 索引に L-134 の 1 行、`session-safety-rules.md` の「ユーザー確認前の基本手順」に秘密検知ゲートで止まったときの対処 1 段落、`session-compression-rules.md` の自動保全コミット記述を「秘密の疑いは除外」に言い換え。いずれも実観測ベースの行動規範（下流で秘密の誤コミットが多発した実害への対処）で、機械強制レイヤ（ゲート自体）は「止める」までしか担えず「止まったら無効化せず秘密を外す」の判断は Hot に要る |
