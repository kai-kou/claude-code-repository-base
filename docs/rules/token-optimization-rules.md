...
| 日付 | 実測 | 差分 | 追加の正当化 / 相殺 |
|---|---:|---:|---|
| 2026-09-02 | 78,724 B | 基準 | **#504 の再棚卸し後の到達値**（増減ログ 4 行到達の合図を受けての再点検）。前回基準（2026-08-23・77,828B）からの増分 +1.6% は超過閾値（+10%）に遠く、実測とログ最新行の乖離もゼロだったため、no-op テスト + SSOT 軸を主軸に 13 本を再点検した。削減できたのは代替の強制レイヤ（フック・他 Hot 層ファイル）と逐語重複していた 2 箇所: ① `pr-review-flow-summary.md` 項目 0 の自動保全コミット書き換え手順（`git commit --amend` / `git reset --soft` / `git push --force-with-lease` の具体コマンド列）が `pre-pr-create-check.sh` のブロックメッセージ自体に同一手順が案内済みだったため要旨 1 行へ圧縮 ② `session-concurrency-rules.md`「二面モデル」の `check_pending_pr_reviews.py` コマンド例が `pr-review-flow-summary.md`「セッション復帰（PR 放置検出）」と完全重複していたため後者への参照 1 行に統一。残る 11 本（`agent-team-summary.md` / `completion-report-rules.md` / `core-principles.md` / `datetime-rules.md` / `lessons-core.md` / `session-compression-rules.md` / `session-safety-rules.md` / `session-sprint-rules.md` / `user-confirmation-minimization.md` / `user-instruction-issue-rules.md` / `user-notification-triage.md`）は削減対象外 3 区分（A-1〜A-6・実観測ベースの lessons L-065/L-077/L-100/L-103/L-111/L-112/L-113/L-124・Haiku 明示テンプレート）に該当する記述が大半で、機械強制レイヤの有無を個別確認したうえで代替が無いか既に要旨のみの状態だったため残置と判断した（例: `pre-git-push-check.sh` は main 直接 push を止めるが復旧手順までは案内しないため L-065 は残置。#483/#494 で検証済みの箇所は再検証せず据え置いた）。増減ログをここで打ち切り、本行を新基準とする（`tools/check_hot_budget.py` で機械検証） |
| 2026-09-03 | 85,124 B | +6,400 B | **#543 完了報告の重複対策**: `completion-report-rules.md` §1.2（Stop フック差し戻し後の続行ターンの規律・SSOT）を新設し、`CLAUDE.md` 完了報告節に 1 行ポインタを追加。続行ターンは final message ごとに構造的に発生するため、Hot 層に置かないと圧縮後に規律が失われて再発する（output style にも 1 行転記）。相殺: なし（§1.2 は良い例/悪い例を含めて 1 節のみ・detail 版への分離は判定表が Hot 層に無いと機能しないため見送り） |
| 2026-09-03 | 85,320 B | +196 B | Warm 層 lesson `docs/rules/lessons/permissions.md`（L-127・auto モードの権限判定）新設に伴う `lessons-core.md` Warm 層索引への 1 行追加（症状 → 参照先）。**本体（事象・原因・2 パターン・対策・罠・未検証）は Warm 層に置き、Hot 層へは昇格させない**（症状を観測したときだけ Read すれば足りるため）。常駐分を索引 1 行に限定したうえで、行動規範そのものは `pre-tool-use-router.sh` の再帰 grep 正規化と `settings.json` の read-only allow 列挙で **機械強制** しており、Hot 層に文章として常駐させる必要がない（削減対象外②「実観測ベースの行動規範」に該当するが、代替の強制レイヤが存在するケース） **訂正（2026-09-04・#558）**: 再帰 grep 正規化（`grep_exclude_normalize.py`）は v2.1.259 に効かず v2.1.260 では不要と実測で確定し撤去した。機械強制の根拠は失効し、以後の担保は `tools/probe_permission_prompts.sh`（新バージョン検知時の実測）と Warm 層 L-127 の行動規範。Hot 層は索引 1 行のまま変更なし |
| 2026-09-05 | 85,521 B | +201 B | Warm 層 lesson `docs/rules/lessons/permissions.md`（L-129・無人ルーティンが作業領域外の Bash 承認プロンプトで無限停止する）新設に伴う `lessons-core.md` Warm 層索引 1 行と、`CLAUDE.md`「やってはいけないこと」への行動規範 1 行。**本体（3 層の根本原因・不採用案・残余リスク）は Warm 層に置き Hot 層へ昇格させない**。Hot 層の 1 行は「一時ファイルは scratchpad へ / persisted output はネイティブツールで読む」という着手前に効く規範で、`pre-tool-use-router.sh` の `workspace_write_guard.py` が機械ブロックする層（事後）と役割が違うため両方を置いた（ブロックは 1 往復のロスを伴い、無人ルーティンではそのロスがそのまま処理遅延になる）。**本行で増減ログが 4 行に到達（再棚卸しの合図 ①）したが、本 PR のスコープ（無人ルーティンの停止防止）と別作業のため #580 に切り出した**（CP-1: スコープ外の改善は Issue 起票までが射程） |

**記載予算（基準）は ~78.7KB / ~19,400 トークン**（#504 再校正・下記ツールが機械検証する）。
