...
| 2026-08-19 | 79,432 B | +10,719 B | **未記録の累積増加を実測で是正**（cookbook 適用検討時に発覚・`content/discussions/cost-optimization-cookbook-adoption/`）。2026-08-04 以降の追加が本ログに 1 行も記録されないまま実測だけが膨張していた（発覚時点 79,072 B）。同日の追加は早期撤退マーカー（`agent-team-summary.md`・+360 B。解説は Warm の `agent-team.md` へ退避）のみで、残る ~10.4KB は本ログ未記録分。**再棚卸しの合図が立っている状態**（削減自体は #469） |

**記載予算は ~68.7KB / ~17,200 トークン、直近実測は 79.4KB（約 16% 超過中・再棚卸しは #469）**。

**再棚卸しの合図**（どちらか一方を満たせば発火）: ① 増減ログが 3 行を超えて増え続ける（累積の合図・#146 で見落とした点） ② **実測が記載予算を 10% 以上超過する**（今回の発火要因）。
