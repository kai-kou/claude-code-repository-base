...
| 2026-09-03 | 85,320 B | +196 B | Warm 層 lesson `docs/rules/lessons/permissions.md`（L-127・auto モードの権限判定）新設に伴う `lessons-core.md` Warm 層索引への 1 行追加（症状 → 参照先）。**本体（事象・原因・2 パターン・対策・罠・未検証）は Warm 層に置き、Hot 層へは昇格させない**（症状を観測したときだけ Read すれば足りるため）。常駐分を索引 1 行に限定したうえで、行動規範そのものは `pre-tool-use-router.sh` の再帰 grep 正規化と `settings.json` の read-only allow 列挙で **機械強制** しており、Hot 層に文章として常駐させる必要がない（削減対象外②「実観測ベースの行動規範」に該当するが、代替の強制レイヤが存在するケース） |
