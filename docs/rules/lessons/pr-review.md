...
クラウド（一次経路・L-114）:
```
mcp__github__list_pull_requests(owner="__OWNER__", repo="__REPO__",
                                head="__OWNER__:{branch}", state="all")
```

ローカル実行用:
```bash
gh pr list --head {branch} -R __OWNER__/__REPO__ --limit 1 --json number,url,state \
