...
```bash
gh variable set SLACK_BOT_TOKEN -R __OWNER__/__REPO__ --body "xoxb-..."
gh variable set SLACK_CHANNEL_ID -R __OWNER__/__REPO__ --body "C0XXXXXXXXX"
# ... 残りの変数
```
...
```bash
gh variable set XXX -R __OWNER__/__REPO__ --body "値"
```
...
# gh CLI で直接操作
gh variable get SLACK_BOT_TOKEN -R __OWNER__/__REPO__
gh variable set SLACK_BOT_TOKEN -R __OWNER__/__REPO__ --body "xoxb-..."
gh variable delete SLACK_BOT_TOKEN -R __OWNER__/__REPO__
```
...
GH_TOKEN で gh CLI 認証（Claude.ai 環境変数から取得）
  ↓
gh variable list -R __OWNER__/__REPO__ --json name,value
  ↓
各変数について:
