...
git remote → __OWNER__/__REPO__ のまま、の順で解決する（優先順位の実装は tools/repo_slug.py が正本）。
"""
...
REPO = resolve_repo_slug("__OWNER__/__REPO__", env_vars=("PROJECT_REPO", "GITHUB_REPOSITORY"))
...
def render_remote(now: str) -> str:
    ...
    **クラウドでは実 gh が未導入のため、これが通常経路になる**（repo スコープ REST の可否は変動する別軸・#692）
