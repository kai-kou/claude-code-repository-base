...
検出条件:
  1. Open状態のPR（__OWNER__/__REPO__）
...
_REPO_PLACEHOLDER = "__OWNER__/__REPO__"
# owner / name を REPO から動的導出する（GraphQL クエリ等でハードコードしない）。
# bootstrap.sh が REPO の __OWNER__/__REPO__ を置換すれば OWNER/REPO_NAME も追従する。
...
def _validate_repo() -> None:
    ...
    # "__OWNER__" 等の完全一致リテラルを書くと bootstrap.sh の全域 sed がこの判定文字列
...
def _run_self_test() -> None:
    ...
    total_cases = len(cases) + 1 + len(branch_cases) + len(assoc_cases) + len(verdict_cases)
    if failures:
