...
def _repo_slug_from_env() -> str | None:
    ...
    slug = resolve_repo_slug(
        "__OWNER__/__REPO__", env_vars=("PROJECT_REPO", "GITHUB_REPOSITORY")
    )
...
ERROR_GUIDANCE: list[tuple[str, str]] = [
    (
        # 2026-09-18 にプロキシが返し始めた新文言（#692）。旧シグネチャとは語が重ならないため
    ...
    (
        ...
        "リモートブランチ削除はオーナーが GitHub UI かローカル git で行う（#399）",
    ),
    (
...
def annotate_and_exit(stdout: bytes, stderr: bytes, code: int) -> None:
    ...
    if code != 0:
        for signature, guidance in ERROR_GUIDANCE:
            ...
            # プロキシの JSON エラーには "HTTP 403" が含まれないことがある（#692 実測）。
...
def self_test() -> None:
    ...
    # 実プロキシ文言でのマッチ検証（シグネチャの存在だけでなく実際に当たるかを見る・#692）。
...
def main() -> None:
    ...
    if not real_gh:
        ...
        if is_cloud():
            ...
            # 直叩きの可否は「プロキシの許可範囲が今どうなっているか」次第で変動する（#692）:
